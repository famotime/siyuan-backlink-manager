...
- `docs/project-structure.md`：已更新，补充 collector helper 与测试入口
- `README.md`：已更新，同步数据层结构说明
- 最终同步检查：已完成，文档内容与 `RF-008` 后的代码结构一致

## 7. 下一步

1. 若继续缩减 `backlink-data.ts`，下一步可考虑把 `getBacklinkBlockArray`、`getHeadlineChildBlockArray`、`getListItemChildBlockArray`、`getParentBlockArray` 这几段查询准备逻辑再拆成 query helper。
